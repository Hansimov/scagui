...
%    1.20 2017-01-19: added display of class origin information
%    1.21 2017-07-07: fixed report for some .Net classes that are reported as A*B*C
%    1.22 2017-09-12: handled a few additional edge-cases
%    1.23 2017-09-13: attribution of whereisjavaclassloadingfrom created by Andrew Janke

% License to use and modify this code is granted freely to all interested, as long as the original author is
% referenced and attributed as such. The original author maintains the right to be solely associated with this work.

% Programmed and Copyright by Yair M. Altman: altmany(at)gmail.com
% $Revision: 1.22 $  $Date: 2017/09/13 14:12:27 $
...
%#ok<*AGROW>
  ...
  %if isempty(obj), obj = class(origObj); end
  if isjava(obj)
  ...
  elseif isempty(obj) || ~isobject(obj)
  ...
  try
      ...
      try
          ...
          isMcos = true;
          if ~isempty(origObj), obj = origObj; end
      catch
          try
  ...
  % Display class origin information: http://undocumentedmatlab.com/blog/static-java-classpath-hacks
  try whereisjavaclassloadingfrom(thisClassName); catch, end
    ...
    function [objMethods,full] = getMethods(obj)
      try
          [objMethods,full] = methods(obj,'-full');
          if isempty(full)
              full = objMethods;
              return
          else
      ...
      for methodIdx = 1 : size(full,1)
          methodStr = [full{methodIdx,4},full{methodIdx,5}];
          if ~isempty(full{methodIdx,2}) && ~strcmp(full{methodIdx,2},'void')
              methodStr = [methodStr ' : ' full{methodIdx,2}];
          end
          if ~isempty(full{methodIdx,1})
              methodStr = [methodStr ' (' full{methodIdx,1} ')'];
          end
          objMethods{methodIdx,1} = regexprep(methodStr,'[^(]*\.','','once');
      end
    end  % getMethods
    ...
    function values = diffValues(title,thisClassName,thisValues,superClassName,superValues, flag, varargin)
      ...
      try superValuesSimple = regexprep(superValues,' [(].*',''); catch, superValuesSimple=''; end
    ...
    function dispValues(title,classname,values,obj)  %#ok used
      try
          ...
          values(values=='*') = '.';  % Yair 7/7/17 - some .Net classes are reported as A*B*C
    ...
    function values = getMcosFields(thisClass,mcosFieldName)
        ...
        if ~isempty(values)
            ...
            for idx = 1 : numel(values)
                try
                    try thisValue = values{idx}; catch, thisValue = values(idx); end
    ...
    function whereisjavaclassloadingfrom(ClassName)
        ...
        % (c) Andrew Janke http://stackoverflow.com/questions/4376565/java-jpa-class-for-matlab#4380622
        % http://undocumentedmatlab.com/blog/static-java-classpath-hacks#analyze
        ...
        klassLoaderStr = char(klassLoader);
        ...
        fprintf('ClassLoader: %s\n', klassLoaderStr);
        if ~isempty(strfind(klassUrlStr,'file:'))
            filename = regexprep(klassUrlStr,'.*file:/?/?','');
            filename = regexprep(filename,'%20',' ');
            filename = regexprep(filename,'!.*','');
            filename = strrep(filename,'/','\');
            fprintf('File: <a href="matlab:winopen(''%s'')">%s</a>\n',filename,filename);
        end
        fprintf('URL: %s\n', klassUrlStr);
        if numel(urls) > 1
            %disp('Masked (shaddowed):');
            for i = 2:numel(urls)
                fprintf('URL: %s (masked/shaddowed)\n', char(urls(i)));
            end
        end
        fprintf('\n');
